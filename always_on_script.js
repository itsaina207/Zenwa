/**
 * Script Always-On pour Zenwa (version autonome)
 * Permet de maintenir l'application active 24/7 sur Replit
 * Ce script est conçu pour être exécuté en tant que workflow séparé
 */
...
const FLASK_PORT = 5000; // Port principal de l'application Flask
const NODE_PORT = 8000; // Port du serveur API Node.js
let pingCount = 0;
...
async function checkFlaskHealth() {
  try {
    const result = await sendRequest(`http://localhost:${FLASK_PORT}/health`);
...
async function checkNodeHealth() {
  try {
    const result = await sendRequest(`http://localhost:${NODE_PORT}/health`);
    return result.success;
  } catch (error) {
...
function checkSystemResources() {
  try {
    ...
    const totalMemoryMB = Math.round(parseInt(process.env.REPL_MEMORY || '512') * 1024);
    const memoryPercentage = Math.round((usedMemoryMB / totalMemoryMB) * 100);
    
    console.log(`[SYSTEM] Memory Usage: ${usedMemoryMB}MB / ${totalMemoryMB}MB (${memoryPercentage}%)`);
    
    // Récupérer la charge CPU (approximative dans cet environnement)
    const uptime = process.uptime();
    const uptimeHours = Math.floor(uptime / 3600);
    const uptimeMinutes = Math.floor((uptime % 3600) / 60);
    console.log(`[SYSTEM] Uptime: ${uptimeHours} hours, ${uptimeMinutes} minutes`);
    
    // Alerte si la mémoire est trop haute
    if (usedMemoryMB > 500) {
      console.warn(`[WARNING] Memory usage (${usedMemoryMB}MB) is above threshold (500MB)`);
      console.log('[KEEP-ALIVE] System resources may be running low');
    }
    
    return { usedMemoryMB, totalMemoryMB, memoryPercentage, uptime };
  } catch (error) {
    console.error('[SYSTEM] Error checking system resources:', error);
    return {};
  }
}
...
console.log('==================================================');
console.log('[ALWAYS-ON] Démarrage du workflow Always-On pour Zenwa');
console.log(`[ALWAYS-ON] Intervalle de ping: ${PING_INTERVAL / 60000} minutes`);
console.log(`[ALWAYS-ON] Ports surveillés: Flask (${FLASK_PORT}), Node.js (${NODE_PORT})`);
console.log('==================================================');
